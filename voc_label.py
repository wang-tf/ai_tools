...
class DarknetDataset():
    ...
    def convert_annotation(self, year, image_id, voc_root_path):
        in_file = open(os.path.join(voc_root_path, 'VOC%s/Annotations/%s.xml'%(year, image_id)))
        out_file = open(os.path.join(voc_root_path, 'VOC%s/labels/%s.txt'%(year, image_id)), 'w')
    ...
    def voc2(self, voc_root_path):
        ...
        for year, image_set in self.voc_sets:
            if not os.path.exists(os.path.join(voc_root_path, 'VOC%s/labels/'%(year))):
                os.makedirs(os.path.join(voc_root_path, 'VOC%s/labels/'%(year)))
            if not os.path.exists(os.path.join(voc_root_path, 'VOC%s/ImageSets/Main/%s.txt'%(year, image_set))):
                continue

            image_ids = open(os.path.join(voc_root_path, 'VOC%s/ImageSets/Main/%s.txt'%(year, image_set))).read().strip().split()
            with open(os.path.join(voc_root_path, '%s_%s.txt'%(year, image_set)), 'w') as list_file:
                image_file_list = []
                for image_id in image_ids:
                    image_file_path = os.path.abspath(os.path.join(voc_root_path, 'VOC{}/JPEGImages/{}.jpg'.format(year, image_id)))
